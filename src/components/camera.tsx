...
const lerp = (a: Vec3, b: Vec3, t: number) => a.map((_, idx) => (1.0 - t) * a[idx] + t * b[idx]);

// sphericaly interpolate between two positions [x,y,z] based on time
const slerp = (a: Vec3, b: Vec3, t: number) => {
  const omega = 0.075;
  ...
  return a.map(
    (_, idx) =>
      (Math.sin((1 - t) * omega) / Math.sin(omega)) * a[idx] + (Math.sin(t * omega) / Math.sin(omega)) * b[idx]
  );
};
...
const easeQuad = {
  ...
  inOut: (x: number): number => (x < 0.5 ? 2 * x * x : 1 - Math.pow(-2 * x + 2, 2) / 2),
};
...
export const Camera: Component<{
  ...
  const frame = (time: number) => {
    ...
    const params = [
      ...
      props.translate.end,
      props.translate.easing ? easeQuad[props.translate.easing](animationTime) : animationTime,
    ] as const;
    const position = props.translate.type === "line" ? lerp(...params) : slerp(...params);

    updateCameraPosition(position, props.translate.target);

    if (!props.translate.loop && (animationTime > 1.0 || animationTime < 0.0)) isReverse = !isReverse;
